...
update_status ModuleRender::Update()
{
    ...
    if (App->game->finishedOnce)(TextFormat("Best time: %.2f", App->game->bestTime), camera.target.x - SCREEN_WIDTH / 2, camera.target.y - SCREEN_HEIGHT / 2 + 40, GetFontDefault(), 20, WHITE, 30);
    //BeginMode2D(camera);
	return UPDATE_CONTINUE;
}
